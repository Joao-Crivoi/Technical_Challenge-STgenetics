...
public class SandwichSideStrategy : IDiscountStrategy
{
    public bool CanApply(IEnumerable<OrderItem> items) =>
        items.Count() == 2 && 
        items.Any(i => i.Product.Category == ProductCategory.Sandwich) &&
        items.Any(i => i.Product.Category == ProductCategory.Side);

    public decimal Calculate(decimal subtotal) => subtotal * 0.10m;
}